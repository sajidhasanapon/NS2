...
echo 'Please enter "1" for 802.11 and "2" for 802.15.4'
read mac
...
do
	...
	do
		...
		awk -f AWK.awk TRACE.tr > $tempFile
...
while [ $i -ge 2 ]
do
	gnuplot -persist -e "set title '802.15.4 : ${arr[i]} vs $param'; set xlabel '$param'; set ylabel '${arr2[i]}'; plot 'out/GRAPH' using 1:$i with lines"
	i=$(($i-1))
done
